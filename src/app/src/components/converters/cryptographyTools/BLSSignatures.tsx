...
const curveOptions = ["secp256k1", "ed25519", "bls12-381"];
const dsaOptions = ["ECDSA", "Schnorr", "BLS DSA"];
const hashOptions = ["SHA256", "SHA3", "Poseidon"];

const BLSSignatures: React.FC = () => {
  ...
  const generateKeys = () => {
    ...
    } else if (selectedDSA === "BLS DSA" && selectedCurve === "bls12-381") {
      priv = schnorr.utils.randomPrivateKey();
      pub = bls.getPublicKey(priv);
    }
  ...
  const signMessage = async () => {
    ...
    } else if (selectedCurve === "bls12-381" && selectedDSA === "BLS DSA") {
      sig = bls.sign(msg, privateKey);
    }
  ...
  const verifySignature = () => {
    ...
    if (selectedCurve === "secp256k1") {
      ...
      const signature = bls.sign(hashedMessage, privateKey);
      const publicKey = bls.getPublicKey(privateKey);
      isValid = bls.verify(signature, hashedMessage, publicKey);
    }
  ...
  const verifyMessage = () => {
    ...
    try {
      ...
      if (selectedCurve === "secp256k1") {
        ...
        const signature = bls.sign(hashedMessage, privateKey);
      const publicKey = bls.getPublicKey(privateKey);
      isVerified = bls.verify(signature, hashedMessage, publicKey);
      }
            ...
            <div>
              <select
                ...
                onChange={(e) => {
                  ...
                  if (e.target.value === "BLS DSA") {
                    setSelectedCurve("bls12-381");
                  }
                }}
              >
