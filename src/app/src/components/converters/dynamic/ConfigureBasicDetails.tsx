...
const ConfigureBasicDetails: React.FC = () => {
  ...
  // Function to process and handle multiple file uploads
  const handleFileUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Loop through uploaded files to categorize them
    Array.from(files).forEach((file) => {
      if (file.name.endsWith(".json")) {
        jsonFile = file;
      } else if (file.name.endsWith(".js")) {
        actionFiles[file.name] = file;
      }
    });
    ...
    // Process JSON file and replace codeRef with actual JS code
    const reader = new FileReader();
    reader.onload = async (e) => {
      if (e.target && typeof e.target.result === "string") {
        try {
          ...
          // Replace all "codeRef" with the actual code from JS files
          const updatedJsonData = await replaceCodeRefsWithCode(jsonData, actionFiles);
          ...
          const updatedFormData = {
            ...existingData,
            title: updatedJsonData.name,
            description: updatedJsonData.description,
            contractDetails: updatedJsonData.contracts,
            networkDetails: updatedJsonData.network,
          };

          saveDataToLocalStorage("formData", updatedFormData);
          saveDataToLocalStorage("components", updatedJsonData.components);

          // Redirect to preview page after processing
          // window.location.href = "/app/new/preview";
  ...
  // Helper function to replace codeRef with actual code from uploaded JS files
  const replaceCodeRefsWithCode = async (jsonData, actionFiles) => {
    const updatedData = { ...jsonData };

    const processComponent = async (component) => {
      if (component.codeRef && actionFiles[component.codeRef]) {
        const jsFile = actionFiles[component.codeRef];
        const jsCode = await readFileAsText(jsFile);
        component.code = jsCode; // Replace codeRef with the actual code
        delete component.codeRef; // Remove codeRef after replacement
      }
      return component;
    };

    // Process components and replace codeRef with code
    if (updatedData.components && Array.isArray(updatedData.components)) {
      updatedData.components = await Promise.all(
        updatedData.components.map((component) => processComponent(component))
      );
    }
  ...
  // Helper function to read the content of a JS file
          ...
          <div className="p-1 md:p-4 flex flex-col gap-5">
            ...
            <div className="flex flex-col">
              ...
              <input
                type="text"
                maxLength={32}
                className="focus:outline-none border rounded-lg p-3 bg-gray-100"
                placeholder="Enter app title.."
                value={title}
                onChange={(e) => setTitle(e.target.value)}
              />
            ...
            <div className="flex flex-col gap-2 md:flex-row md:gap-0 md:items-center md:justify-between mt-4">
              <div className="flex flex-col">
              ...
              <div className="flex justify-end">
                <Link to="#" onClick={handleSaveNext} className="mx-0">
                  <button
                    className="cursor-pointer text-white bg-green-600 rounded-md p-3 font-semibold"
                    type="submit"
                  >
                    Save & Next
                  </button>
              ...
              <div>
                <button onClick={() => window.location.href = "/app/new/preview"}>preview</button>
              </div>
            </div>
          </div>
        ) : (
          <div className="flex flex-col items-center justify-center p-3 py-5">
            <p className="text-gray-600 mb-4">You need to log in to create custom apps.</p>
            <a href={`https://github.com/login/oauth/authorize?client_id=${GITHUB_CLIENT_ID}&scope=user:email`}>
              <button className="cursor-pointer text-white bg-green-600 rounded-md p-3 font-semibold">
                Login with GitHub
              </button>
            </a>
          </div>
