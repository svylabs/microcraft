...
const GraphComponent: React.FC<Props> = ({ output, configurations }) => {
  ...
  const renderGraph = () => {
    ...
    if (config) {
      ...
      if (output) {
        ...
        const dataValues = output.map((value: any) => parseFloat(value[1]));
        const dataLabels = output.map((value: any) => value[0]);
    ...
    <>
      {output && config ? (
        ...
        <pre className="overflow-auto w-full mt-2 px-4 py-2 bg-gray-100 overflow-x-auto border border-gray-300 rounded-lg">
          No output available for Graph.</pre>
      )}
    </>
  );
};
