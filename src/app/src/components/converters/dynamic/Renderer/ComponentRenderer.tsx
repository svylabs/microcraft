...
interface Props {
  ...
  handleRun: (code: string, data: any) => void;
}

const ComponentRenderer: React.FC<Props> = ({ components, data, handleInputChange, handleRun }) => {
  return (
        ...
        <li key={index} className="mb-4">
          ...
          {(component.type === "text" || component.type === "number" || component.type === "file") && (
            <input
              ...
              style={{ ...(component.inputConfig ? JSON.parse(component.inputConfig) : {}) }}
              type={component.type}
              id={component.id}
          ...
          {component.type === "dropdown" && (
            ...
            >
              {component.optionsConfig &&
                JSON.parse(component.optionsConfig).values.map((option, idx) => (
                  ...
                  return (
                    ...
                    >
                      <input
                        type="checkbox"
                        id={`${component.id}_${idx}`}
                        checked={data[component.id] && data[component.id].includes(option)}
                        onChange={(e) => {
          ...
          {component.type === "slider" && (
            <div className="flex items-center gap-3">
              <input
                ...
                value={data[component.id] || JSON.parse(component.sliderConfig).value}
                onChange={(e) => handleInputChange(component.id, e.target.value)}
              />
              <span className="font-semibold">
                {data[component.id] || JSON.parse(component.sliderConfig).value}
              </span>
            </div>
          )}
          {component.type === "walletDropdown" && (
            <div>
              <Wallet
                ...
                onSelectAddress={(address) => handleInputChange(component.id, { address, balance: null })}
